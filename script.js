  ...
  for(let i=1;i<=11;i++){
    ...
    if(i<=10){
      if(totalScore>=levelUnlockScore[i-1]){
        ...
        btn.innerText = `🔒 Level ${i}`; btn.disabled=true; btn.classList.add("locked");
      }
    } else {
      btn.innerText=`Level 11 (Coming Soon)`; btn.disabled=true; btn.classList.add("locked");
    }
...
function startGame(level){
  ...
  gamePaused=false;
  fallSpeed = 5 + level; 
  spawnDelay = Math.max(300, 1000-50*level);
...
gameDiv.addEventListener("mousemove", e=>{ if(isGameOver||gamePaused) return; updatePlayer(e.clientX - gameDiv.getBoundingClientRect().left); });
gameDiv.addEventListener("touchmove", e=>{
  e.preventDefault();
  if(isGameOver||gamePaused) return; 
  const touch = e.touches[0];
  updatePlayer(touch.clientX - gameDiv.getBoundingClientRect().left);
},{passive:false});
